...
/*
	For unordered STL contaiers like std::unordered_set and std::unordered_map, we will define a comparator which can simply determine uniqueness. Two connections are unique if and only if each of the 4 numbers in the 4-tuple are equal.
*/
class EqCmpCT {
 public:
 	bool operator()(const ConnectionTuple& a, const ConnectionTuple& b) const noexcept {
...
int main (int argc, char* argv[])
{
	...
	std::cout << "a==b? " << EqCmpCT()(a,b) << std::endl;
	std::cout << "a==d? " << EqCmpCT()(a,d) << std::endl;
	std::cout << "c==d? " << EqCmpCT()(c,d) << std::endl;
	std::cout << "c==e? " << EqCmpCT()(c,e) << std::endl;
	...
	set.insert(a);
	std::cout << "set size after insert a: " << set.size() << std::endl;
	set.insert(b);
	std::cout << "set size after insert b: " << set.size() << std::endl;
	set.insert(c);
	std::cout << "set size after insert c: " << set.size() << std::endl;
	set.insert(d);
	std::cout << "set size after insert d: " << set.size() << std::endl;
	set.insert(e);
	std::cout << "set size after insert e: " << set.size() << std::endl;
	std::cout << std::endl;
	...
	u_set.insert(a);
	std::cout << "u_set size after insert a: " << u_set.size() << std::endl;
	u_set.insert(b);
	std::cout << "u_set size after insert b: " << u_set.size() << std::endl;
	u_set.insert(c);
	std::cout << "u_set size after insert c: " << u_set.size() << std::endl;
	u_set.insert(d);
	std::cout << "u_set size after insert d: " << u_set.size() << std::endl;
	u_set.insert(e);
	std::cout << "u_set size after insert e: " << u_set.size() << std::endl;
	std::cout << std::endl;

	std::cout << "Let's try out our hash function" << std::endl;
	std::hash<ConnectionTuple> hf;
	std::cout << hf(a) << std::endl;
	std::cout << hf(b) << std::endl;
	std::cout << hf(c) << std::endl;
	std::cout << hf(d) << std::endl;
	std::cout << hf(e) << std::endl;
	std::cout << std::endl;
	...
	std::cout << "u_map[a] -> ";
	it == u_map.end() ? std::cout << "n/a" : std::cout << it->second;
	std::cout << std::endl;
